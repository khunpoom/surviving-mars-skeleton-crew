...
Drop the `SkeletonCrew` folder into:

```
%AppData%\Surviving Mars Relaunched\Mods\SkeletonCrew
```

Enable it in the Mod Manager, then **fully restart the game**.

Options: Game Options → Mod Options → Skeleton Crew

Safe to add to an existing save. Removing it hides the upgrade on buildings that have not built it yet; already-built upgrades stay until you turn them off.
